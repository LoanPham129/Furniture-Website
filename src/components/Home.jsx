...
const Home = () => {
  const bedProduct = {
    id: 100,
    title: "bed",
    name: "Luxury Soft Bed",
    img: bed,
    price: 1500,
  };

  const { cartItems, addToCart, updateQuantity } = useCart();
  const cartItem = cartItems.find((item) => item.id === bedProduct.id);
          ...
          <div className="w-full flex md:block justify-center">
            <div className="flex-col items-center justify-center md:absolute md:top-1/2 md:right-24 lg:-translate-y-3/3 md:-translate-y-2/3 h-fit backdrop-blur-sm w-[300px] md:w-[300px] lg:w-[400px] p-5 mt-5 bg-gray-200/20 rounded-3xl text-black text-xs md:text-xl">
              ...
              <p className="pb-5 pl-3 ">
                Cotton, slik and Bamboo are hypoallergenic and breathable,
                making them good choices for people with sensitive skin.
              </p>
                  ...
                  <div className="flex items-center gap-2">
                    {cartItem ? (
                      <>
                        <button
                          onClick={() =>
                            updateQuantity(bedProduct.id, cartItem.quantity - 1)
                          }
                          className="w-8 h-8 rounded-full bg-amber-800 text-white flex items-center justify-center hover:bg-amber-600 transition"
                        >
                          <FaMinus size={12} />
                        </button>
                      ...
                      <div className="w-12 h-12 bg-amber-800 rounded-full flex items-center justify-center">
                        <FaShoppingBag
                          onClick={() => addToCart(bedProduct, 1)}
                          className="shake-hover text-white w-6 h-6 cursor-pointer"
                        />
                      </div>
                    )}
                  </div>
